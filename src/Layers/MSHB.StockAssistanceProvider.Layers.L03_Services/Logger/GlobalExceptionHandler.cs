...
namespace MSHB.StockAssistanceProvider.Layers.L03.Services.Logger
{
    public static class GlobalExceptionHandler
    {
            ...
            app.UseExceptionHandler(builder =>
            {
                builder.Run(async context =>
                {
                    ...
                    if (ex != null)
                        ...
                        {
                            case "StockAssistanceProviderGlobalException":
                                StockAssistanceProviderGlobalExceptionHandler(ex, context);
                                break;
        ...
        private static async void StockAssistanceProviderGlobalExceptionHandler(IExceptionHandlerFeature ex, HttpContext context)
        {
            StockAssistanceProviderGlobalException exception = (StockAssistanceProviderGlobalException)ex.Error;
            ...
            List<StockAssistanceProviderErrorMessage> detailErrorList = GetExceptionErrors(exception.ExceptionList);
        ...
        private static List<StockAssistanceProviderErrorMessage> GetExceptionErrors(List<Exception> exceptions)
        {

            List<StockAssistanceProviderErrorMessage> detailErrorList = new List<StockAssistanceProviderErrorMessage>();
            if (exceptions != null && exceptions.Count > 0)
            {
                foreach (Exception ex in exceptions)
                {
                    if (ex is StockAssistanceProviderGlobalException exception)
                    {
                        detailErrorList.Add(new StockAssistanceProviderErrorMessage(exception.ErrorCode, exception.UserMessage));
                    ...
                    {
                        detailErrorList.Add(new StockAssistanceProviderErrorMessage("EXC-500", ex.Message));
                    }
                }
